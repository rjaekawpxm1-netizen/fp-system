...
${keyword
  ? `"${keyword}" 관련 기능목록 생성.`
  : `시스템명과 개요를 분석하여 해당 시스템에 필요한 전체 기능목록을 생성.`
}

## 생성 규칙
1. LV1(대분류): 시스템의 주요 업무 영역
2. LV2(중분류): LV1 하위의 세부 업무단위
3. LV3(소분류): 실제 단위 기능 (CRUD 패턴 적용)

## 반드시 포함할 공통 기능
- 사용자관리 (계정등록/수정/삭제/조회, 권한관리)
- 시스템관리 (공통코드, 메뉴관리, 로그관리)
- 주요 업무별 통계/현황 기능

## 업무단위별 표준패턴
각 LV2 업무단위에는 반드시:
등록(EI), 수정(EI), 삭제(EI), 목록조회(EQ), 상세조회(EQ) 포함
필요시: 승인처리(EI), 통계(EO), 현황조회(EQ) 추가

## 참고 예시
${FEW_SHOT_EXAMPLES.split('### 패턴')[0]}

definition은 "~을/를 ~한다" 형식으로 작성.
