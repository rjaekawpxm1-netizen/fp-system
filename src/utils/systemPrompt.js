...
## LV3 동사 (반드시 아래 중 하나로 끝낼 것)
등록|수정|삭제|목록조회|상세조회|검색|처리|승인|반려|제출|취소|확정|출력|통계조회|현황조회|이력조회|설정|초기화|활성|비활성|배정|알림

## Few-shot 예시

### 예시1: 연동관리 > 연동합의서관리
→ 연동합의서 등록 / 연동합의서 수정 / 연동합의서 삭제
  연동합의서 목록조회 / 연동합의서 상세조회
  연동합의서 검토처리 / 연동합의서 승인처리 / 첨부파일 업로드

### 예시2: 사업관리 > 제안수주관리
→ 제안서 등록 / 제안서 수정 / 제안서 삭제
  제안서 목록조회 / 제안서 상세조회
  수주 처리 / 수주 반려 / 수주현황 통계조회

### 예시3: 시스템관리 > 사용자관리
→ 사용자 등록 / 사용자 수정 / 사용자 삭제
  사용자 목록조회 / 사용자 상세조회
  비밀번호 초기화 / 계정 활성 / 계정 비활성 / 권한 설정
...
export const getFPPrompt = (functions) => {
  const compact = functions.map((f, i) => ({
    i, l2: f.lv2, l3: (f.lv3 || '').slice(0, 15),
  }));
  return `${FP_CORE}
${FP_WEIGHTS_TABLE}

## FP 유형 분류 기준
- ILF: 시스템이 관리하는 내부 데이터 그룹 (LV2 단위로 1개 배정)
- EIF: 외부 시스템 참조 데이터
- EI: 데이터 입력/수정/삭제 → 등록,수정,삭제,처리,승인,반려,제출,확정,설정
- EO: 데이터 가공 출력 → 통계,보고서,집계,출력,현황,그래프
- EQ: 단순 조회 → 조회,검색,목록,상세,확인,이력
