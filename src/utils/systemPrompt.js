...
- 행정 절차 (납품, 검수, 평가기준)
- 하드웨어/네트워크/인프라 구축
- 기술 방법론 (AI/ML 적용, AIOps 도입, 클라우드 전환, 아키텍처 설계)
- "~자동화 구현", "~지능화", "~고도화" 형태의 기술 목표
- 다른 시스템 요구사항
...
LV1 = 시스템 상단 메뉴바의 버튼 이름
예시: [사업관리] [내부행정] [현황관리] [시스템관리]
...
### 규칙
- LV1 개수: 5~10개 (메뉴바에 들어가는 항목 수)
- 반드시 사용자가 클릭하는 메뉴명으로 작성
- 기술용어(API, SLA, 메타데이터, AIOps) → LV1 금지, LV2에 배치
- 유사한 업무는 하나의 LV1으로 묶기
- 공통기능(사용자관리/권한관리/시스템관리) 항상 포함

### 절대 금지 LV1
❌ 메타데이터관리, SLA관리, API관리, 아키텍처설계
❌ AI/ML, AIOps, 클라우드인프라, 실시간스트리밍
❌ 너무 세부적인 것 (LV2 수준을 LV1으로 올리지 말 것)

{"domains":[{"lv1":"연동관리","description":"연동 업무 전반","requirements":["관련요구사항"],"expectedLv2":["연동계획관리","연동운영관리","연동설정관리"]}]}
`;
...
## 절대 생성 금지 패턴 (실제 오류 사례)
❌ "~자동화", "~지능화", "~고도화" 단독 LV3 → 기술 개념
❌ "AI 분석 실행", "ML 모델 적용" → 기술 인프라
❌ "아키텍처 설계", "인프라 구성" → 컨설팅 과업
❌ "정책 수립", "전략 수립" → 컨설팅 과업
❌ LV3 = LV2와 동일한 이름 → 의미없는 중복
✅ 올바른 예: "신청 등록", "승인 처리", "현황 조회", "통계 출력"

## CRUD 강제 규칙 (모든 LV2에 반드시 적용)
데이터 관리 LV2: 반드시 등록/수정/삭제/목록조회/상세조회 포함
처리 LV2: 반드시 처리/승인/반려 + 이력조회 포함
조회 LV2: 목록조회/상세조회/검색 + 출력 포함
통계 LV2: 통계조회/현황조회/집계출력 포함

## LV2 작성 규칙 (메뉴 2단계)
LV2 = LV1 클릭 후 나오는 서브메뉴 항목
예: 연동관리 > [연동계획관리] [연동운영관리] [연동현황조회]
- 업무 중분류 기준 (여전히 메뉴 관점)
- 기술용어를 LV2에서는 허용 (SLA관리, API관리 등)
- LV2당 LV3 6~10개 생성

## LV3 동사 규칙 (반드시 아래 중 하나로 끝낼 것)
등록 | 수정 | 삭제 | 목록조회 | 상세조회 | 검색 | 처리 |
승인 | 반려 | 제출 | 취소 | 확정 | 출력 | 통계조회 | 현황조회 |
이력조회 | 설정 | 초기화 | 활성 | 비활성 | 배정 | 알림

## LV3 명칭 규칙
- [업무명사] + [위의 동사]: "출입신청 등록", "연동현황 조회"
- 15자 이내, 한국어만, 코드번호 절대 금지
