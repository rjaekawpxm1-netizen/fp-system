...
const ProjectDetail = ({ projects, onUpdateProject }) => {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      if (file.type === 'image/png' || file.type === 'image/jpeg' || file.name.endsWith('.jpg')) {
        imageFile = file;
  ...
  const handleGenerateFP = async () => {
    ...
    setLoading(true);
    setLoadingMsg('AI가 FP 산정 중... (기능 수에 따라 1~2분 소요)');
    try {
  ...
  const updateFP = (fid, field, value) => {
    const updated = fpList.map((f) => f.id === fid ? { ...f, [field]: value } : f);
    setFpList(updated);
  ...
  const addFPRow = () => {
    const updated = [...fpList, { id: Date.now(), lv1: '', lv2: '', lv3: '', definition: '', fpType: 'EQ', ftr: 1, det: 5, reuseType: '신규개발', ftrChange: 0, detChange: 0 }];
    setFpList(updated);
  ...
  const cellStyle = { padding: '6px 8px', borderBottom: '1px solid #e5e7eb', fontSize: 13, verticalAlign: 'middle' };
  ...
  return (
    <div style={{ maxWidth: 1400, margin: '0 auto', padding: '24px 16px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          {[
            ...
            <div key={s.label} style={{ background: s.bg, color: s.color, borderRadius: 8, padding: '8px 16px', textAlign: 'center', minWidth: 100 }}>
      ...
      {tab === 'setup' && (
        <div>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
                  {['연동계획', '연동운영', '연동관제', '체계운영', '체계관리'].map((kw) => (
                    <button key={kw} onClick={() => setKeyword(kw)} style={{ padding: '4px 12px', borderRadius: 20, fontSize: 12, background: '#eff6ff', color: '#2563eb', border: '1px solid #bfdbfe', cursor: 'pointer' }}>
                      {kw}
                    </button>
                  ))}
                </div>
          ...
          {inputMethod === 'file' && (
            <div style={{ maxWidth: 500 }}>
              ...
              <label style={{ display: 'block', border: '2px dashed #93c5fd', borderRadius: 10, padding: '32px', textAlign: 'center', cursor: 'pointer', background: '#eff6ff' }}>
                ...
                <p style={{ fontWeight: 600, color: '#2563eb' }}>PDF / DOCX / PNG / JPG 업로드</p>
                <p style={{ fontSize: 12, color: '#6b7280', marginTop: 4 }}>클릭하여 파일 선택</p>
                <input type="file" accept=".pdf,.docx,.png,.jpg,.jpeg" onChange={handleFileUpload} style={{ display: 'none' }} />
              </label>
          ...
          {inputMethod === 'funcdef' && (
            <div style={{ maxWidth: 500 }}>
              <p style={{ fontSize: 14, color: '#374151', marginBottom: 12 }}>
                기능정의서를 업로드하면 LV1~LV3를 자동 추출하고 바로 FP 산정으로 이동합니다.<br />
              ...
              <thead>
                <tr style={{ background: '#f8fafc' }}>
                  {['LV1', 'LV2', 'LV3', '기능 정의', '삭제'].map((h) => (
                    <th key={h} style={{ ...cellStyle, fontWeight: 600, textAlign: 'left', color: '#374151', borderBottom: '2px solid #e5e7eb' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr key={f.id}>
                    {['lv1', 'lv2', 'lv3', 'definition'].map((field) => (
                      <td key={field} style={cellStyle}>
                        <input value={f[field] || ''} onChange={(e) => updateFunction(f.id, field, e.target.value)} style={{ width: '100%', border: 'none', outline: 'none', fontSize: 13, background: 'transparent', padding: '2px 4px' }} />
                      </td>
                    ))}
                    <td style={cellStyle}>
      ...
      {tab === 'fp' && (
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <p style={{ fontSize: 14, color: '#6b7280' }}>총 {fpList.length}개 기능 · 모든 셀 수정 가능 · FTR/DET 수정 시 가중치 자동 재계산</p>
                ...
                {fpList.map((f) => {
                  ...
                  return (
                    <tr key={f.id}>
                      ...
                      {['ftr','det'].map((field) => (
                        <td key={field} style={cellStyle}>
                          <input type="number" value={f[field] || 0} onChange={(e) => updateFP(f.id, field, Number(e.target.value))} style={{ width: 50, border: '1px solid #d1d5db', borderRadius: 4, padding: '2px 4px', fontSize: 12 }} />
                        </td>
                      ))}
                      <td style={{ ...cellStyle, fontWeight: 700, color: '#2563eb', textAlign: 'center' }}>{weight}</td>
                      <td style={cellStyle}>
                        <select value={f.reuseType} onChange={(e) => updateFP(f.id, 'reuseType', e.target.value)} style={{ fontSize: 12, border: '1px solid #d1d5db', borderRadius: 4, padding: '2px 4px', background: rColor.bg, color: rColor.color, fontWeight: 500 }}>
